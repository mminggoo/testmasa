...
class AttentionStore(AttentionBase):
    ...
    def after_step(self):
        if self.cur_step > self.min_step and self.cur_step < self.max_step and len(self.self_attns_step) > 0:
            self.valid_steps += 1
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        if attn.requires_grad and attn.shape[1] <= 32 ** 2:  # avoid OOM
            if is_cross:
    ...
    def get_average_attention(self, is_cross):
        attn_maps = self.cross_attns if is_cross else self.self_attns
        average_attention = [item / self.cur_step for item in attn_maps]
        return average_attention

    def aggregate_attention(
        self, res, from_where, is_cross, select
    ):
        out = []
        attention_maps = self.get_average_attention(is_cross)
        num_pixels = res**2

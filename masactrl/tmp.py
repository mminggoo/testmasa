    ...
    def __call__(
        ...
        **kwds):
        ...
        for i, t in enumerate(tqdm(self.scheduler.timesteps, desc="DDIM Sampler")):
            ...
            new_latents = latents[1:].clone()
            latent_prev = ref_intermediate_latents[-2 - i]
            new_latents.requires_grad_(True)
            for _ in range(1):
                with torch.enable_grad():
                    new_latents.requires_grad_(True)
                    loss = F.mse_loss(new_latents*(1-mask), latent_prev*(1-mask)) # l1_loss mse_loss
                grad_cond = torch.autograd.grad(loss.requires_grad_(True), [new_latents])[0]
                new_latents = new_latents - grad_cond*grad_scale

            torch.cuda.empty_cache()
            new_latents = new_latents.requires_grad_(False)
            latents[1:] = new_latents
